<template>
	<div class="new-paper">
    <div class="top-title">{{ paperTitle }}</div>
    <!-- <h2 class="migu-content-title" style='font-family: "Microsoft YaHei";font-weight:200;'>{{ title }}</h2> -->
    <el-form :inline="true" size="small" :rules="rules2">
		  <el-form-item label="试卷名称：" style="margin-right:50px;">
		    ...
		    <!--<el-input v-else class="w290" v-model="examPaperRequest.paperName" placeholder="请在这里输入试卷名称，不得超过20个字" @blur="EditPaperName"></el-input>-->
			  <input v-else class = "w290 examinput" maxlength="20" ref="input" v-model.trim="examPaperRequest.paperName" placeholder="请在这里输入试卷名称，不得超过20个字" @blur="EditPaperName" />
		  </el-form-item>
    ...
    <div v-show="examPaperRequest.questionRequest.length === 0" class="add-title-question">
      <el-row>
		...
		<div class="sumary-area">
      	<h3 class="migu-content-title" style="margin-bottom: 5px">试题详情</h3>
        <el-table size="small" :data="summary" style="width: 100%">
		    ...
		    <div class="align-center mt20">
				  <el-button class="CreatePaperBtn" type="primary" round @click="generatePaper" :disabled="creatingPaper">生成试卷</el-button>
			  </div>
		</div>
  ...
  export default {
    ...
    created () {
      ...
      if(this.$route.query.id){
        ...
        Api.getPaperDetailInfo({
          paperId: this.$route.query.id
        }).then((res) => {
          // 传给组件的字段
          let paperInfo = res.data
          // 替换字段数据
          paperInfo.questionNumRequestList = paperInfo.questionNumList
          delete paperInfo.questionNumList
          paperInfo.questionRequest = paperInfo.questionList
          delete paperInfo.questionList
          paperInfo.questionRequest.forEach((item, index) => {
            paperInfo.questionRequest[index].questionOptionRequestList = item.optionList
            delete paperInfo.questionRequest[index].optionList
          })
          this.examPaperRequest = paperInfo
        })
      ...
      if(this.$route.query.isImport) {
        let importData = JSON.parse(window.localStorage.getItem("importPaperInfo"))
        this.examPaperRequest = importData
      }
      // if(this.type === '2'){
    ...
    methods:{
      ...
      generatePaper () {
        ...
        this.examPaperRequest.paperAuthor = sessionStorage.getItem('adminAccount')
        if(this.testParam()) {
          if(this.$route.query.id) {
            this.examPaperRequest.paperId = this.$route.query.id
            this.examPaperRequest.source = 2
            this.creatingPaper = true
            this.updatePaperInFo(this.examPaperRequest).then(res => {
                if(res.code === 0) {
      ...
      ...mapActions(["getCreateBlankPaperInfo","getExamDetailInfo", "addPaperInFo", "updatePaperInFo"])
    }
  }
</script>
...
<style lang="less">
  ...
  .new-paper {
    ...
    .choose-title {
	  ...
	  .el-card__body .el-row .el-col-22 span{
		  vertical-align:top;
		  // width:90%;
