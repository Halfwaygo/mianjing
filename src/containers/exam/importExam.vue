      ...
      <el-row>
        <el-col :span="4" :offset="4">
            <div class="step-wrap">
              ...
              <h3>1、下载模板</h3>
              <p class="align-left">模板中包含如下信息：试卷名称、单选题、多选题等信息</p>
        ...
        <el-col :span="4" :offset="2">
            <div class="step-wrap">
              ...
              <h3>3、上传导入</h3>
              <p class="align-left">导入后，试卷将存入试卷库</p>
            </div>
        </el-col>
      </el-row>
    </el-card>
      ...
      uploadSuccess (response, file, fileList) {
        if(response.code === 0){ //上传成功，进入成功状态ok
          ...
          if(response.data.questionList.length === 0) {
            this.$message.warning('导入的试题不能为空, 请从新导入')
            this.state = 'init'
            return false
          }
          ...
          if(this.$route.query.type === 'exam') {
            window.localStorage.setItem("importExamInfo",str)
            this.$router.push('newExam?isImport=1')
          } else if(this.$route.query.type === 'paper') {
            // this.$router.push('paperLibrary')
          let str = JSON.stringify(response.data).replace(/questionNumList/g,"questionNumRequestList").replace(/questionList/g,"questionRequest").replace(/optionList/g,"questionOptionRequestList");
          window.localStorage.setItem("importPaperInfo",str)
          this.$router.push('newPaper?isImport=1')
          }
        }else { //上传失败，进入失败状态
          this.state = 'fail'
