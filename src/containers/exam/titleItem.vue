<template>
  <div class="title-item-style">
		...
		<div class="fix-question">
		        <img v-show="canEidt" @click="editTitle" src="../../layouts/images/saveblue.png" alt="保存" style="width: 21px;cursor: pointer;"/>
						<el-button v-if="!canEidt" @click="editTitle"
								type="handle edit"
								size="small"
								title="编辑">
						</el-button>
			    <span>
					...
					<span v-show="!isquestion">
						<img v-if="dataLength-1 !== idx " @click="moveQue(1, idx)" src="../../layouts/images/1bottom.png" alt="下移" style="margin-left: 5px;cursor: pointer;"/>
						<img v-else src="../../layouts/images/1bottomdisable.png" alt="下移" style="margin-left: 5px;"/>
					</span>
					<span v-show="!isquestion">
						<img v-if="idx !== 0" @click="moveQue(2, idx)" src="../../layouts/images/1top.png" alt="上移" style="margin-left: 5px;cursor: pointer;"/>
						<img v-else src="../../layouts/images/1topdisable.png" alt="上移" style="margin-left: 5px;"/>
					</span>
			</div>
		...
		<template v-if="canEidt">
			...
			<el-row>
				...
				<el-col :span="5">
					<el-input v-model.trim="titleItem.questionDesc" :maxlength="250" placeholder="请输入题目"></el-input>
				</el-col>
				<el-col :span="2" class="align-right">分值：</el-col>
				<el-col :span="5"> <el-input v-model.trim="titleItem.questionScore"  type="number" @blur="testScore"></el-input></el-col>
			</el-row>
			<el-row v-for="(item,key) in titleItem.questionOptionRequestList" :key="key">
				...
				<el-col :span="5"><el-input v-model.trim="item.optionDesc" :placeholder="'选项'+(key+1)" :maxlength="250"></el-input></el-col>
				<el-col :span="5" class="clearfix">
						<span class="titleItemError fl ml10 fs32 " @click="removeOpt(key)">×</span>
						<el-radio v-if="titleItem.typeId === 1 || titleItem.typeId === '1'" v-model="answerSingle" :label="String(item.optionSort)" class="ml35">
							设为答案
						</el-radio>
						<template v-else>
							...
							<el-checkbox v-else @change='handleChange' :label="item.optionSort" class="ml35">
								设为答案
							</el-checkbox>
						</template>
				</el-col>
			...
			<el-row>
				...
				<el-col :span="20">
					<knowledge :showAdd="true" @surePoint="surePoint" :getlevels="getLevels" :zswatch="zswatch"></knowledge>
				</el-col>
			</el-row>
			<el-row>
			...
			<el-row class="que-bank">
				  <!-- 编辑 -->
					...
					<el-col :span="1" class="fl">
						<span class=" color-blue" @click="editTitle"></span><br/>
						<span class="color-danger" @click="deleteTitle"></span>
					</el-col>
					<el-col :span="20" class="fl">
						<el-row>
							<el-col :span="22">
								<span style="float:left">{{titleItem.typeId == 1?"【单选题】":"【多选题】"}}</span>
								<span class="item-title">{{titleItem.questionDesc}}</span>
							</el-col>
						...
						<el-row v-for="(item,key) in titleItem.questionOptionRequestList" :key="key">
							<el-col :span="10" class="clearfix">
								...
								<div v-if="titleItem.typeId ==1 ">
									<el-radio  style="position: absolute;" v-model="answerSingle" :label="String(item.optionSort)" disabled class="ml35">
										{{ wordList[key] }}
									</el-radio>
								...
								<!-- 多选 -->
								<template v-else>
									<el-checkbox  v-show="answerList.indexOf(String(item.optionSort)) !== -1" @change='handleChange' disabled checked :label="''" class="ml35">
										{{ '' }}
									</el-checkbox>
									<el-checkbox v-show="answerList.indexOf(String(item.optionSort)) === -1" @change='handleChange' :label="''" class="ml35" disabled>
										{{ '' }}
										</el-checkbox>
  	...
  	watch: {
        ...
        this.titleItem.questionOptionRequestList.forEach(res => {
        	res.optionInRight = res.optionSort === newValue ? 1 : 0
				...
				// if(!this.$route.query.isImport && !this.$route.query.id && this.$route.path !== '/operationAnalysis/importQuestionSave') {
				// 	// 新建的时候不走这个方法
				//
       ...
       handler(newVal, oldVal) {
				...
				// 正确答案复制
				this.titleItem = newVal;
				// this.answerSingle =  newVal.questionAnswer*1
				if(newVal.questionAnswer.split(',').length > 1) {
           this.answerList = newVal.questionAnswer.split(",")
				} else {
					//
					// console.log(JSON.stringify(oldVal));
					if(newVal.questionAnswer !== '') {
             this.answerSingle = newVal.questionAnswer
					}

				}
				// if(newVal.typeId === '2' && !isNaN(newVal.questionAnswer)) {
				// 	this.answerList = newVal.questionAnswer.split(",")
        ...
        if(newVal.questionPointInfoList === undefined) {
          return
				...
				if(JSON.stringify(this.changeKnowParam) !== JSON.stringify(this.source.questionPointInfoList) || this.isMoveQus === true) {
					 let levObj = {}
					newVal.questionPointInfoList.forEach((res, index) => {
					if(index === 0) {
						levObj.level_one = res.pointId
						} else if(index === 1) {
							levObj.level_two = res.pointId
						} else {
							levObj.level_three = res.pointId
						}
					})
					this.getLevels = levObj

					console.log(JSON.stringify(this.getLevels));

					this.isMoveQus = false
				}
				// this.getLevels.level_one = this.source.pointId
			 },
			 deep: true
		 },
	  },
    data () {
      return {
        ...
        questionScore:'',
				...
				isMoveQus: false,
				// 此参数用于判断编辑时所属知识点和换位置时所属知识点是否一样
				changeKnowParam: [],
				imgs : {
					saveImg: require('../../layouts/images/saveblue.png')
				}
      }
    },
		...
		mounted() {
			// 将正确答案赋值
			this.answerSingle =  String(this.source.questionAnswer*1)
    ...
    methods:{
          ...
          this.$emit('setAddTitle',type, this.idx+1)
				...
				surePoint (item) {
					...
					console.log(JSON.stringify(item.questionPointInfoList));

					this.titleItem.questionPointInfoList = item.questionPointInfoList
					this.changeKnowParam = item.questionPointInfoList
				},
				removeOpt (key) {
				...
				addOpt () {
					...
					this.titleItem.questionOptionRequestList.forEach((item, index) => {
						this.titleItem.questionOptionRequestList[index].optionSort  = index +1
					})
				},
				deleteTitle () {
				...
				editTitle () {
					...
					// console.log(this.titleItem.typeId)
					this.zswatch = false
					this.canEidt = !this.canEidt
				},
				testScore () {
